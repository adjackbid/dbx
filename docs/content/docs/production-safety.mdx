...
| Per-account MCP policy | Restricts the MCP clients of one account through a connection allowlist and three execution modes | Client environment variables cannot widen the saved account policy |
...
DBX stores one MCP policy per account under **Settings → MCP**, and every MCP request reloads the policy of the account the client authenticated as.
...
Configure **Allowed connections** in addition to the execution mode. Connections outside the allowlist are not exposed to MCP. Both settings belong to the signed-in account, so each account only exposes its own connections. These limits remain active in every mode:
