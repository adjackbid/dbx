...
| `version` | Version of the running server (shown on the sign-in screen) |
| `commit` | Git revision of the build, up to 12 characters (`unknown` when unavailable) |
| `buildTimeMs` | Build time as Unix milliseconds |
...
This route requires a session; `GET /api/auth/check` returns the same three fields without one, which is
what the sign-in screen uses to display the running build. `commit` is the git revision embedded at
build time (see `CONTRIBUTING.md` → Versioning).
