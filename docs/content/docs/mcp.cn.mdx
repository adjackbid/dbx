...
<Steps>
  ...
  <Step>
    ...
    如果使用全局安装，可将 `command` 改为 `dbx-mcp-server`。连接 allowlist 和执行权限按登录账号在 **DBX 设置 → MCP** 中管理；DBX Web 中每个账号各自维护自己的 allowlist 与执行权限。常规客户端配置不需要权限环境变量。Windows 便携版请将 `DBX_DATA_DIR` 设置为 `DBX.exe` 同级的 `data` 目录。
  </Step>
  <Step>
    ### 开始使用
...
DBX 在 **设置 → MCP** 中按登录账号各保存一份 MCP 策略，并在每次请求时重新读取。MCP 客户端使用它登录账号的策略，因此连接 allowlist 和执行权限始终属于该账号：
...
新版 Server 不允许 `DBX_MCP_ALLOW_WRITES` 或 `DBX_MCP_ALLOW_DANGEROUS_SQL` 放宽 DBX 策略。为兼容升级，在账号策略首次保存前，旧配置中的 `DBX_MCP_ALLOW_WRITES=0`（或 `false`）仍会保持 MCP 只读；策略保存后仅以该账号策略为准，并忽略旧权限变量。旧连接 scope 变量只能进一步收窄 DBX allowlist。
