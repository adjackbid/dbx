...
| `version` | 正在运行的服务端版本（显示在登录页） |
| `commit` | 构建的 Git 版本号，最多 12 个字符（无法取得时为 `unknown`） |
| `buildTimeMs` | 构建时间的 Unix 毫秒值 |
...
该路由需要会话；`GET /api/auth/check` 无需会话即可返回同样三个字段，登录页正是用它显示当前构建。
`commit` 是构建时写入的 Git 版本号（参见 `CONTRIBUTING.md` 的 Versioning 一节）。
