...
当前文档站语言以 `docs/lib/i18n.ts`、`docs/scripts/languages.mjs` 为准（现为 `en` / `cn`）。
...
<Callout type="warn">
  只要改动会影响用户可见行为，就必须同时跳四个清单档的应用版本号：`package.json`、`src-tauri/tauri.conf.json`、`src-tauri/Cargo.toml`、`crates/dbx-web/Cargo.toml`，以及 `Cargo.lock` 中对应的两个 `version`（`dbx` 与 `dbx-web`）。Docker 镜像会通过 `DBX_BUILD_COMMIT` 写入 git 版本号，登录画面会显示版本、revision 与构建时间，运维人员据此确认部署实际运行的版本。详见仓库内 `docs/account-isolation.md` 第 10 节。
</Callout>
