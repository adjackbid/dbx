...
mod tests {
    ...
    fn test_inject_single_script() {
        ...
        assert_eq!(result, "<html><head><script>console.log('injected');</script></head><body>Content</body></html>");
    }
    ...
    fn test_inject_script_creates_head_if_missing() {
        ...
        assert_eq!(result, "<html><head><script>console.log('test');</script></head><body>Content</body></html>");
    }
    ...
    fn test_csp_header_updated_with_script_hashes() {
        ...
        response.headers_mut().insert(CONTENT_SECURITY_POLICY, HeaderValue::from_static("default-src 'self'"));

        let script_code = "console.log('test');";
        let scripts = vec![script_code.to_string()];

        let scripts: Vec<InitializationScript> =
            scripts.into_iter().map(|script| InitializationScript { script, for_main_frame_only: true }).collect();
        ...
        assert_eq!(result_body, "<html><head><script>console.log('test');</script></head><body>Content</body></html>");
        assert_eq!(csp_str, "default-src 'self' script-src 'sha256-3x8DE279hr8o/Aq0dEdH4WApIwn5rbRKhugPzn6Bofw='");
    }

    fn create_response(body: &str, content_type: &'static str) -> HttpResponse<Cow<'static, [u8]>> {
        let mut response =
            HttpResponse::builder().status(StatusCode::OK).body(Cow::Owned(body.as_bytes().to_vec())).unwrap();
