...
use objc2_foundation::{
    NSData, NSHTTPURLResponse, NSMutableDictionary, NSObject, NSObjectProtocol, NSString, NSURL, NSUUID,
};
    ...
    unsafe {
        ...
        match cls {
            Some(mut cls) => {
                ...
                cls.add_method(objc2::sel!(webView:startURLSchemeTask:), start_task as extern "C" fn(_, _, _, _));
                cls.add_method(objc2::sel!(webView:stopURLSchemeTask:), stop_task as extern "C" fn(_, _, _, _));
                cls.register()
            }
...
extern "C" fn start_task(
    ...
    unsafe {
        ...
        let webview_id = CStr::from_ptr(webview_id_ptr).to_str().ok().unwrap_or_default();

        let ivar = this.class().instance_variable(c"protocol_index").unwrap();
        let protocol_index: usize = *ivar.load(this);

        let function =
            WEBVIEW_STATE.read().unwrap().get(webview_id).and_then(|v| v.protocol_ptrs.get(protocol_index)).cloned();
        ...
        if let Some(function) = function {
            ...
            let respond_with_404 = || {
                ...
                let response = NSHTTPURLResponse::initWithURL_statusCode_HTTPVersion_headerFields(
                    ...
                    Some(&NSString::from_str(format!("{:#?}", Version::HTTP_11).as_str())),
                    None,
                )
                .unwrap();
            ...
            match http_request.body(sent_form_body) {
                Ok(final_request) => {
                    ...
                    let responder: Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> = Box::new(move |sent_response| {
                        ...
                        unsafe fn response(
                            ...
                            for (name, value) in sent_response.headers().iter() {
                                if let Ok(value) = value.to_str() {
                                    headers.insert(&*NSString::from_str(name.as_str()), &*NSString::from_str(value));
                                }
                            }
                        ...
                        if let Err(_e) = response(task, webview, task_key, task_uuid, webview_id, url, sent_response) {
                            #[cfg(feature = "tracing")]
                    ...
                    function(webview_id, final_request, RequestAsyncResponder { responder });
                }
                Err(_) => respond_with_404(),
            };
        } else {
            #[cfg(feature = "tracing")]
            tracing::warn!("Either WebView or WebContext instance is dropped! This handler shouldn't be called.");
        };
    }
}
