...
define_class!(
    ...
    unsafe impl WKScriptMessageHandler for WryWebViewDelegate {
        ...
        fn did_receive(this: &WryWebViewDelegate, _controller: &WKUserContentController, msg: &WKScriptMessage) {
            ...
            unsafe {
                ...
                if let Ok(body) = body.downcast::<NSString>() {
                    ...
                    if let (Ok(url), Ok(js)) = (CStr::from_ptr(url_utf8).to_str(), CStr::from_ptr(js_utf8).to_str()) {
...
impl WryWebViewDelegate {
    pub fn new(
        ...
        let delegate = mtm.alloc::<WryWebViewDelegate>().set_ivars(WryWebViewDelegateIvars { ipc_handler, controller });
