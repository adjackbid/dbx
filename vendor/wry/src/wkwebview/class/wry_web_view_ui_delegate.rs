...
use objc2_web_kit::{WKFrameInfo, WKMediaCaptureType, WKPermissionDecision, WKSecurityOrigin, WKUIDelegate};
...
impl WryNSWindowDelegate {
    pub fn new(mtm: MainThreadMarker, on_close: Box<dyn Fn()>) -> Retained<Self> {
        let delegate = mtm.alloc::<WryNSWindowDelegate>().set_ivars(WryNSWindowDelegateIvars { on_close });
        ...
        unsafe fn create_web_view_for_navigation_action(
            ...
            if let Some(new_window_req_handler) = &self.ivars().new_window_req_handler {
                ...
                match new_window_req_handler(
                    url.to_string(),
                    NewWindowFeatures {
                        size: if let (Some(width), Some(height)) = (window_features.width(), window_features.height()) {
                            Some(dpi::LogicalSize::new(width.doubleValue(), height.doubleValue()))
                        } else {
                            None
                        },
                    ...
                    NewWindowResponse::Allow => {
                        ...
                        let size = objc2_foundation::NSSize::new(
                            window_features.width().map_or(defaults.size.width, |width| width.doubleValue()),
                            window_features.height().map_or(defaults.size.height, |height| height.doubleValue()),
                        );
                        let position = objc2_foundation::NSPoint::new(
                            window_features.x().map_or(defaults.origin.x, |x| x.doubleValue()),
                        ...
                        let resizable =
                            window_features.allowsResizing().map_or(true, |resizable| resizable.boolValue());
                        if resizable {
