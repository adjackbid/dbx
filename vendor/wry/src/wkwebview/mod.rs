...
use objc2_foundation::{
    ns_string, MainThreadMarker, NSArray, NSBundle, NSDate, NSError, NSHTTPCookie, NSHTTPCookieDomain,
    NSHTTPCookieExpires, NSHTTPCookieMaximumAge, NSHTTPCookieName, NSHTTPCookiePath, NSHTTPCookiePropertyKey,
    NSHTTPCookieSecure, NSHTTPCookieValue, NSHTTPCookieVersion, NSJSONSerialization, NSMutableDictionary,
    NSMutableURLRequest, NSNumber, NSObjectNSKeyValueCoding, NSObjectProtocol, NSString, NSUTF8StringEncoding, NSURL,
    NSUUID,
};
#[cfg(target_os = "ios")]
...
use objc2_web_kit::{
    WKAudiovisualMediaTypes, WKInactiveSchedulingPolicy, WKURLSchemeHandler, WKUserContentController, WKUserScript,
...
use crate::{
    proxy::ProxyConfig,
    wkwebview::proxy::{nw_endpoint_t, nw_proxy_config_create_http_connect, nw_proxy_config_create_socksv5},
};

use crate::{BackgroundThrottlingPolicy, Error, Rect, RequestAsyncResponder, Result, WebViewAttributes, RGBA};
...
impl InnerWebView {
    ...
    fn new_ns_view(
        ...
        let webview_id = attributes.id.map(|id| id.to_string()).unwrap_or_else(|| COUNTER.next().to_string());
        ...
        unsafe {
            ...
            #[cfg(target_os = "macos")]
            let config = pl_attrs.webview_configuration.unwrap_or_else(|| WKWebViewConfiguration::new(mtm));
            #[cfg(target_os = "ios")]
            ...
            let data_store = if using_existing_config {
                ...
                let data_store =
                    match (attributes.incognito, custom_data_store_available, pl_attrs.data_store_identifier) {
            ...
            for (name, function) in attributes.custom_protocols {
                ...
                let already_registered =
                    using_existing_config && config.urlSchemeHandlerForURLScheme(&NSString::from_str(&name)).is_some();
                ...
                let ivar = (*handler).class().instance_variable(c"protocol_index").unwrap();
            ...
            WEBVIEW_STATE.write().unwrap().insert(webview_id.clone(), WebViewState { protocol_ptrs });
            ...
            let webview = {
                ...
                let (x, y) = attributes
                    ...
                    .unwrap_or((0, 0));
                let (w, h) = if is_child {
                    attributes.bounds.map(|b| b.size.to_logical::<u32>(scale_factor)).map(Into::into)
                } else {
                    None
                }
                .unwrap_or_else(|| {
            ...
            #[cfg(target_os = "ios")]
            {
                webview.setAutoresizingMask(UIViewAutoresizing::FlexibleWidth | UIViewAutoresizing::FlexibleHeight);
            ...
            let document_title_changed_observer = if let Some(handler) = attributes.document_title_changed_handler {
            ...
            #[cfg(target_os = "macos")]
            {
                ...
                // Available: macOS 11+
                let can_set_titlebar_style = ns_window.respondsToSelector(objc2::sel!(setTitlebarSeparatorStyle:));
  ...
  value: Object.freeze({postMessage: function(s) {window.webkit.messageHandlers.ipc.postMessage(s);}})
        ...
        if let Some(scripts) = &mut *self.pending_scripts.lock().unwrap() {
            ...
            unsafe {
                ...
                if let Some(callback) = callback {
                    ...
                    #[cfg(feature = "tracing")]
                    let handler = Some(block2::RcBlock::new(move |_val: *mut AnyObject, _err: *mut NSError| {
                        span.lock().unwrap().take();
                    }));
    ...
    fn navigate_to_string(&self, html: &str) {
        // Safety: objc runtime calls are unsafe
        unsafe {
            self.webview.loadHTMLString_baseURL(&NSString::from_str(html), None);
        }
    }

    fn set_user_agent(&self, user_agent: &str) {
        unsafe {
            self.webview.setCustomUserAgent(Some(&NSString::from_str(user_agent)));
        }
    }
    ...
    pub fn print_with_options(&self, _options: &PrintOptions) -> crate::Result<()> {
        ...
        unsafe {
            ...
            // Available: macOS 11+
            let can_print = self.webview.respondsToSelector(objc2::sel!(printOperationWithPrintInfo:));
            if can_print {
    ...
    pub fn set_background_color(&self, _background_color: RGBA) -> Result<()> {
        ...
        unsafe {
            ...
            let no = NSNumber::numberWithBool(false);
            self.webview.setValue_forKey(Some(&no), ns_string!("drawsBackground"));
    ...
    pub fn set_bounds(&self, #[allow(unused)] bounds: Rect) -> crate::Result<()> {
        ...
        if self.is_child {
            ...
            unsafe {
                ...
                let frame =
                    CGRect { origin: window_position(&parent_view, x, y, height), size: CGSize::new(width, height) };
    ...
    unsafe fn cookie_into_wkwebview(cookie: &cookie::Cookie<'_>) -> Retained<NSHTTPCookie> {
        ...
        let domain = cookie.domain().map_or_else(NSString::new, NSString::from_str);

        let properties: Retained<NSMutableDictionary<NSHTTPCookiePropertyKey, AnyObject>> =
            NSMutableDictionary::from_slices(
                &[NSHTTPCookieName, NSHTTPCookieValue, NSHTTPCookiePath, NSHTTPCookieDomain],
        ...
        if let Some(http_only) = cookie.http_only() {
            let http_only = if http_only { nstring_true } else { nstring_false };
            // ref:
    ...
    pub fn cookies(&self) -> Result<Vec<cookie::Cookie<'static>>> {
        ...
        unsafe {
            ...
            // Available: macOS 10.13+, iOS 11+
            self.data_store.httpCookieStore().getAllCookies(&block2::RcBlock::new(
                move |cookies: NonNull<NSArray<NSHTTPCookie>>| {
                    let cookies = cookies.as_ref();
                    let cookies =
                        cookies.to_vec().into_iter().map(|cookie| Self::cookie_from_wkwebview(&cookie)).collect();
    ...
    pub fn set_cookie(&self, cookie: &cookie::Cookie<'_>) -> Result<()> {
        ...
        unsafe {
            ...
            // Available: macOS 10.13+, iOS 11+
            self.data_store.httpCookieStore().setCookie_completionHandler(
                &wkwebview_cookie,
    ...
    pub fn fetch_data_store_identifiers<F: FnOnce(Vec<[u8; 16]>) + Send + 'static>(cb: F) -> crate::Result<()> {
        ...
        let block = block2::RcBlock::new(move |stores: NonNull<NSArray<NSUUID>>| {
            let uuid_list = unsafe { stores.as_ref() }.to_vec().iter().map(|uuid| uuid.as_bytes()).collect();
...
pub fn url_from_webview(webview: &WKWebView) -> Result<String> {
    ...
    std::str::from_utf8(bytes).map(Into::into).map_err(Into::into)
}

pub fn platform_webview_version() -> Result<String> {
    unsafe {
        let Some(bundle) = NSBundle::bundleWithIdentifier(ns_string!("com.apple.WebKit")) else {
            return Err(Error::Io(std::io::Error::other("failed to locate com.apple.WebKit bundle")));
        };
        let Some(dict) = bundle.infoDictionary() else {
            return Err(Error::Io(std::io::Error::other("failed to get WebKit info dictionary")));
        };

        let Some(webkit_version) = dict.objectForKey(ns_string!("CFBundleVersion")) else {
            return Err(Error::Io(std::io::Error::other("failed to get WebKit version")));
        };

        let Ok(webkit_version) = webkit_version.downcast::<NSString>() else {
            return Err(Error::Io(std::io::Error::other("failed to parse WebKit version")));
        };
    ...
    fn drop(&mut self) {
        ...
        unsafe {
            if let Some(ipc_handler) = self.ipc_handler_delegate.take() {
                ...
                ipc_handler.ivars().controller.removeScriptMessageHandlerForName(ipc);
            }
