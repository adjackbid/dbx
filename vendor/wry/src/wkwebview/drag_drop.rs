...
pub(crate) fn dragging_entered(this: &WryWebView, drag_info: &ProtocolObject<dyn NSDraggingInfo>) -> NSDragOperation {
...
pub(crate) fn dragging_updated(this: &WryWebView, drag_info: &ProtocolObject<dyn NSDraggingInfo>) -> NSDragOperation {
    let dl: NSPoint = drag_info.draggingLocation();
    let frame: NSRect = this.frame();
    let position = (dl.x as i32, (frame.size.height - dl.y) as i32);
...
pub(crate) fn perform_drag_operation(this: &WryWebView, drag_info: &ProtocolObject<dyn NSDraggingInfo>) -> Bool {
    let paths = collect_paths(drag_info);
    let dl: NSPoint = drag_info.draggingLocation();
    let frame: NSRect = this.frame();
    let position = (dl.x as i32, (frame.size.height - dl.y) as i32);
