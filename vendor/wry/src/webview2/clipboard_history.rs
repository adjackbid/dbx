...
use windows::Win32::Graphics::Gdi::{CopyEnhMetaFileW, DeleteEnhMetaFile, DeleteObject, HENHMETAFILE, HGDIOBJ};
...
unsafe fn on_clipboard_update(host: HWND) {
    ...
    if REOWN_IN_FLIGHT.compare_exchange(false, true, Ordering::AcqRel, Ordering::Acquire).is_err() {
        return;
    }
...
unsafe fn window_process_image_is(hwnd: HWND, image_name: &str) -> bool {
    ...
    let queried = QueryFullProcessImageNameW(process, PROCESS_NAME_WIN32, PWSTR(path.as_mut_ptr()), &mut len);
...
fn process_image_matches(image_path: &str, image_name: &str) -> bool {
    image_path.rsplit(['\\', '/']).next().is_some_and(|name| name.eq_ignore_ascii_case(image_name))
}
...
unsafe fn snapshot_bytes(format: u32) -> Option<Payload> {
    ...
    if size == 0 {
        return Some(Payload::Bytes { format, data: Vec::new() });
    }
...
unsafe fn snapshot_bitmap(format: u32) -> Option<Payload> {
    ...
    CopyImage(handle, IMAGE_BITMAP, 0, 0, IMAGE_FLAGS(0)).ok().map(Payload::Bitmap)
}
...
mod tests {
    ...
    use windows::Win32::UI::WindowsAndMessaging::{
        CreateWindowExW, DefWindowProcW, DestroyWindow, RegisterClassW, HCURSOR, HICON, WINDOW_EX_STYLE, WNDCLASSW,
        WNDCLASS_STYLES, WS_OVERLAPPEDWINDOW,
    };

    static CLIPBOARD_TEST_LOCK: Mutex<()> = Mutex::new(());

    unsafe extern "system" fn test_wndproc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
    ...
    unsafe fn create_test_window() -> HWND {
        ...
        CLASS_REGISTERED.get_or_init(|| {
            let class = WNDCLASSW {
                ...
                cbClsExtra: 0,
                cbWndExtra: 0,
                hInstance: GetModuleHandleW(PCWSTR::null()).map(Into::into).ok().unwrap_or_default(),
                hIcon: HICON::default(),
    ...
    fn utf16_bytes(text: &str) -> Vec<u8> {
        text.encode_utf16().chain(std::iter::once(0)).flat_map(|unit| unit.to_le_bytes()).collect()
    }
    ...
    fn process_image_matching_is_case_insensitive_file_name_suffix() {
        ...
        assert!(process_image_matches("MSEDGEWEBVIEW2.EXE", WEBVIEW2_BROWSER_PROCESS));
        assert!(process_image_matches("msedgewebview2.exe", WEBVIEW2_BROWSER_PROCESS));
        ...
        assert!(!process_image_matches(r"C:\Windows\System32\notepad.exe", WEBVIEW2_BROWSER_PROCESS));
    ...
    fn reown_roundtrip_preserves_formats_bytes_and_owner() {
        ...
        unsafe {
            ...
            let data_before: Vec<(u32, Vec<u8>)> =
                formats_before.iter().map(|&format| (format, read_clipboard_bytes(format).unwrap())).collect();
            assert!(CloseClipboard().is_ok());

            // Act: the re-own mechanics with the clipboard opened by `window`.
            assert!(open_clipboard(window), "OpenClipboard failed");
            assert!(snapshot_and_reapply(), "HGLOBAL-only clipboards must be re-owned");
            assert!(CloseClipboard().is_ok());
    ...
    fn reown_aborts_on_unsafe_formats_without_data_loss() {
        ...
        unsafe {
            ...
            let logpalette = LOGPALETTE {
                palVersion: 0x300,
                palNumEntries: 1,
                palPalEntry: [PALETTEENTRY { peRed: 1, peGreen: 2, peBlue: 3, peFlags: 0 }],
            };
            ...
            assert!(open_clipboard(window), "OpenClipboard failed");
            assert!(!snapshot_and_reapply(), "re-own must abstain when an unsafe format is present");
            assert!(CloseClipboard().is_ok());
    ...
    fn reown_aborts_when_a_format_fails_to_snapshot() {
        ...
        unsafe {
            ...
            assert!(IsClipboardFormatAvailable(delayed_format).is_ok(), "the delay-rendered format must be registered");
            assert!(CloseClipboard().is_ok());

            // Act: the unsnapshotable format must abort the whole re-own.
            assert!(open_clipboard(window), "OpenClipboard failed");
            assert!(!snapshot_and_reapply(), "a format that fails to snapshot must make the re-own abstain");
            assert!(CloseClipboard().is_ok());
