...
use std::{
    borrow::Cow, cell::RefCell, collections::HashSet, ffi::OsStr, fmt::Write, fs, os::windows::ffi::OsStrExt,
...
use windows::{
    ...
    Win32::{
        ...
        UI::{
            Input::KeyboardAndMouse::{SetFocus, VK_F6},
            Shell::*,
            WindowsAndMessaging::*,
        },
    },
...
use super::Theme;
use crate::{
    custom_protocol_workaround, proxy::ProxyConfig, Error, MemoryUsageLevel, NewWindowFeatures, NewWindowOpener,
    NewWindowResponse, PageLoadEvent, Rect, RequestAsyncResponder, Result, WebView2ProcessFailedInfo,
    WebView2ProcessFailedKind, WebViewAttributes, RGBA,
};
...
impl InnerWebView {
    ...
    fn new_in_hwnd(
        ...
        let id = attributes.id.map(|id| id.to_string()).unwrap_or_else(|| (hwnd.0 as isize).to_string());

        let background_color = if attributes.transparent { Some((0, 0, 0, 0)) } else { attributes.background_color };
        ...
        let webview = Self::init_webview(parent, hwnd, id.clone(), attributes, &env, &controller, pl_attrs, is_child)?;

        let drag_drop_controller = drop_handler.map(|handler| {
            // Disable file drops, so our handler can capture it
            unsafe {
                let _ = controller.cast::<ICoreWebView2Controller4>().and_then(|c| c.SetAllowExternalDrop(false));
            }
            DragDropController::new(hwnd, handler)
        });

        let w =
            Self { id, parent: RefCell::new(parent), hwnd, controller, is_child, webview, env, drag_drop_controller };
    ...
    #[inline]
    fn create_container_hwnd(parent: HWND, attributes: &WebViewAttributes, is_child: bool) -> Result<HWND> {
        unsafe extern "system" fn default_window_proc(hwnd: HWND, msg: u32, wparam: WPARAM, lparam: LPARAM) -> LRESULT {
            if msg == WM_SETFOCUS {
    ...
    fn create_environment(
        ...
        let data_directory =
            attributes.context.as_deref().and_then(|context| context.data_directory()).map(HSTRING::from);
        ...
        unsafe {
            ...
            let browser_executable_folder_ptr =
                browser_executable_folder.as_ref().map(|folder| PCWSTR(folder.as_ptr())).unwrap_or_else(PCWSTR::null);
            ...
            CreateCoreWebView2EnvironmentWithOptions(
                ...
                &CreateCoreWebView2EnvironmentCompletedHandler::create(Box::new(move |error_code, environment| {
                    ...
                    tx.send(result).map_err(|_| windows::core::Error::from(E_UNEXPECTED))
                })),
            )?;
        }
    ...
    fn init_webview(
        ...
        unsafe {
            let accelerator_key_handler = AcceleratorKeyPressedEventHandler::create(Box::new(move |_, args| {
        ...
        let custom_protocols: HashSet<String> = attributes.custom_protocols.iter().map(|n| n.0.clone()).collect();
    ...
    unsafe fn attach_handlers(
        ...
        let new_window_req_handler = attributes.new_window_req_handler.take().map(std::rc::Rc::new);
        let env_ = env.clone();
        // New window handler
                    ...
                    let features = args
                        ...
                        .map(|f| {
                            ...
                            NewWindowFeatures {
                                position,
                                size,
                                opener: NewWindowOpener { webview: webview.clone(), environment: env_.clone() },
                            }
                        })
        ...
        webview.add_WebResourceRequested(
            &WebResourceRequestedEventHandler::create(Box::new(move |_, args| {
                    ...
                    .find(|(protocol, _)| custom_protocol_workaround::is_work_around_uri(&uri, http_or_https, protocol))
                {
                    ...
                    custom_protocol_handler(&webview_id, request, RequestAsyncResponder { responder: async_responder });
                }
    ...
    unsafe fn prepare_request(
        ...
        if let Ok(content) = webview_request.Content() {
            ...
            loop {
                ...
                content.Read(buffer.as_mut_ptr() as *mut _, buffer.len() as u32, Some(&mut cb_read)).ok()?;
        ...
        let path =
            custom_protocol_workaround::revert_uri_work_around(webview_request_uri, http_or_https, custom_protocol);
    ...
    fn parent_bounds(hwnd: HWND) -> Result<PhysicalSize<i32>> {
        ...
        Ok(PhysicalSize::new(client_rect.right - client_rect.left, client_rect.bottom - client_rect.top))
    }
    ...
    unsafe fn dettach_parent_subclass(parent: HWND) {
        ...
        let _ = RemoveWindowSubclass(parent, Some(Self::parent_subclass_proc), PARENT_SUBCLASS_ID as _);
    }
    ...
    fn add_script_to_execute_on_document_created(webview: &ICoreWebView2, js: String) -> Result<()> {
        ...
        AddScriptToExecuteOnDocumentCreatedCompletedHandler::wait_for_async_operation(
            Box::new(move |handler| unsafe {
                ...
                webview.AddScriptToExecuteOnDocumentCreated(&js, &handler).map_err(Into::into)
            }),
            Box::new(|e, _| e),
        )
        .map_err(Into::into)
    ...
    unsafe fn load_extensions(webview: &ICoreWebView2, extension_path: &PathBuf) -> Result<()> {
        let profile = webview.cast::<ICoreWebView2_13>()?.Profile()?.cast::<ICoreWebView2Profile7>()?;
...
impl InnerWebView {
    ...
    pub fn bounds(&self) -> Result<Rect> {
        ...
        if self.is_child {
            ...
            let position_point = &mut [POINT { x: rect.left, y: rect.top }];
    ...
    pub fn set_bounds_inner(&self, size: PhysicalSize<i32>, position: PhysicalPosition<i32>) -> Result<()> {
        unsafe {
            self.controller.SetBounds(RECT { top: 0, left: 0, right: size.width, bottom: size.height })?;
    ...
    pub fn focus(&self) -> Result<()> {
        unsafe { self.controller.MoveFocus(COREWEBVIEW2_MOVE_FOCUS_REASON_PROGRAMMATIC).map_err(Into::into) }
    }
    ...
    unsafe fn cookie_from_win32(cookie: ICoreWebView2Cookie) -> Result<cookie::Cookie<'static>> {
        ...
        let expires = match expires {
            ...
            datetime => {
                cookie::time::OffsetDateTime::from_unix_timestamp(datetime as _).ok().map(cookie::Expiration::DateTime)
            }
        };
    ...
    unsafe fn cookie_into_win32(
        ...
        let expires = if let Some(max_age) = cookie.max_age() {
            let expires_ = cookie::time::OffsetDateTime::now_utc().saturating_add(max_age).unix_timestamp();
            Some(expires_)
        } else {
    ...
    pub fn print(&self) -> Result<()> {
        self.eval("window.print()", None::<Box<dyn FnOnce(String) + Send + 'static>>)
    }
    ...
    pub fn clear_all_browsing_data(&self) -> Result<()> {
        unsafe {
            self.webview
                ...
                .ClearBrowsingDataAll(&ClearBrowsingDataCompletedHandler::create(Box::new(move |_| Ok(()))))
                .map_err(Into::into)
        }
    }
...
#[inline]
unsafe fn set_background_color(controller: &ICoreWebView2Controller, background_color: RGBA) -> Result<()> {
    ...
    controller2.SetDefaultBackgroundColor(COREWEBVIEW2_COLOR { R: r, G: g, B: b, A: a }).map_err(Into::into)
}
...
pub fn platform_webview_version() -> Result<String> {
    ...
    let browser_executable_folder_ptr =
        browser_executable_folder.as_ref().map(|folder| PCWSTR(folder.as_ptr())).unwrap_or_else(PCWSTR::null);
    unsafe { GetAvailableCoreWebView2BrowserVersionString(browser_executable_folder_ptr, &mut versioninfo) }?;
...
fn configured_browser_executable_folder() -> Option<HSTRING> {
    ...
    let fixed_runtime_enabled = should_use_fixed_runtime(cfg!(target_vendor = "win7"), version.major, version.minor);
...
fn process_failed_kind(kind: COREWEBVIEW2_PROCESS_FAILED_KIND) -> WebView2ProcessFailedKind {
    match kind {
        ...
        COREWEBVIEW2_PROCESS_FAILED_KIND_RENDER_PROCESS_UNRESPONSIVE => WebView2ProcessFailedKind::RendererUnresponsive,
        COREWEBVIEW2_PROCESS_FAILED_KIND_FRAME_RENDER_PROCESS_EXITED => WebView2ProcessFailedKind::FrameRenderer,
        ...
        COREWEBVIEW2_PROCESS_FAILED_KIND_SANDBOX_HELPER_PROCESS_EXITED => WebView2ProcessFailedKind::SandboxHelper,
        COREWEBVIEW2_PROCESS_FAILED_KIND_PPAPI_PLUGIN_PROCESS_EXITED => WebView2ProcessFailedKind::PpapiPlugin,
        COREWEBVIEW2_PROCESS_FAILED_KIND_PPAPI_BROKER_PROCESS_EXITED => WebView2ProcessFailedKind::PpapiBroker,
...
fn webview2_process_failed_callback() -> Option<crate::WebView2ProcessFailedCallback> {
    crate::WEBVIEW2_PROCESS_FAILED_CALLBACK.read().ok().and_then(|callback| callback.clone())
}
...
fn browser_executable_folder_from(folder: Option<&OsStr>, fixed_runtime_enabled: bool) -> Option<HSTRING> {
    if !fixed_runtime_enabled {
        return None;
    }
    let folder = folder.filter(|folder| !folder.is_empty())?;
    Some(HSTRING::from_wide(&folder.encode_wide().collect::<Vec<_>>()))
}
...
mod tests {
    ...
    fn process_failed_kind_and_reason_names_are_stable() {
        assert_eq!(process_failed_kind_name(WebView2ProcessFailedKind::Browser), "browser");
        assert_eq!(process_failed_kind_name(WebView2ProcessFailedKind::Renderer), "renderer");
        assert_eq!(process_failed_kind_name(WebView2ProcessFailedKind::RendererUnresponsive), "renderer_unresponsive");
        assert_eq!(process_failed_kind_name(WebView2ProcessFailedKind::FrameRenderer), "frame_renderer");
        assert_eq!(process_failed_kind_name(WebView2ProcessFailedKind::Gpu), "gpu");
        assert_eq!(process_failed_reason_name(COREWEBVIEW2_PROCESS_FAILED_REASON_CRASHED), "crashed");
        assert_eq!(process_failed_reason_name(COREWEBVIEW2_PROCESS_FAILED_REASON_OUT_OF_MEMORY), "out_of_memory");
    }
}
