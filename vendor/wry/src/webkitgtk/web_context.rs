...
use webkit2gtk::{
    ApplicationInfo, AutomationSessionExt, CookiePersistentStorage, DownloadExt, SecurityManagerExt, URIRequest,
    URIRequestExt, URISchemeRequest, URISchemeRequestExt, URISchemeResponse, URISchemeResponseExt, WebContext,
...
impl WebContextImpl {
    ...
    pub fn create_context(context: WebContext) -> Self {
        ...
        Self { context, automation, app_info: Some(app_info) }
    }
...
impl WebContextExt for super::WebContext {
        ...
        F: Fn(crate::WebViewId, Request<Vec<u8>>, RequestAsyncResponder) + 'static,
    {
        ...
        self.os.context.security_manager().ok_or(Error::MissingManager)?.register_uri_scheme_as_secure(name);
        ...
        self.os.context.register_uri_scheme(name, move |request| {
            ...
            if let Some(uri) = request.uri() {
                ...
                let responder: Box<dyn FnOnce(HttpResponse<Cow<'static, [u8]>>)> = Box::new(move |http_response| {
                ...
                request.finish_error(&mut glib::Error::new(glib::FileError::Exist, "Could not get uri."));
            }
        });
                    ...
                    move |download| {
                        if let Some(uri) = download.request().and_then(|req| req.uri()) {
                            ...
                            download_completed_handler(
                                uri,
                                (!failed).then(|| download.destination().map(PathBuf::from)).flatten(),
                                !failed,
                            )
                        }
                    }
