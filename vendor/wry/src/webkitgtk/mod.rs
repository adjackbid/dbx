...
use webkit2gtk::{
    ...
    NavigationPolicyDecisionExt, NetworkProxyMode, NetworkProxySettings, PolicyDecisionType, PrintOperationExt,
    SettingsExt, URIRequest, URIRequestExt, UserContentInjectedFrames, UserContentManager, UserContentManagerExt,
    UserScript, UserScriptInjectionTime, WebContextExt as Webkit2gtkWeContextExt, WebView, WebViewExt,
    WebsiteDataManagerExt, WebsiteDataManagerExtManual, WebsitePolicies,
};
use webkit2gtk_sys::{
    webkit_get_major_version, webkit_get_micro_version, webkit_get_minor_version, webkit_policy_decision_ignore,
...
use crate::{
    proxy::ProxyConfig, web_context::WebContext, Error, NewWindowFeatures, NewWindowOpener, NewWindowResponse,
...
impl InnerWebView {
    ...
    fn new_x11<W: HasWindowHandle>(
        ...
        Self::new_gtk(&vbox, attributes, pl_attrs).map(|mut w| {
            ...
            w.x11.replace(X11Data { is_child, xlib, x11_display: x11_display as _, x11_window, gtk_window });
    ...
    fn create_container_x11_window(
        ...
        let (x, y) =
            attributes.bounds.map(|b| b.position.to_physical::<f64>(scale_factor)).map(Into::into).unwrap_or((0, 0));
    ...
    #[cfg(feature = "x11")]
    pub fn create_gtk_window(raw: *mut GdkX11Display, x11_window: c_ulong) -> (gtk::Window, gtk::Box) {
        // Gdk.Window
        ...
        W: IsA<gtk::Container>,
    {
        ...
        let id = attributes.id.map(|id| id.to_string()).unwrap_or_else(|| (webview.as_ptr() as isize).to_string());
    ...
    fn create_webview(
        ...
        if attributes.autoplay {
            builder = builder.website_policies(&WebsitePolicies::builder().autoplay(AutoplayPolicy::Allow).build());
        }
    ...
    fn attach_handlers(webview: &WebView, web_context: &mut WebContext, attributes: &mut WebViewAttributes) {
        // window.close()
        ...
        if let Some(new_window_req_handler) = attributes.new_window_req_handler.take() {
            ...
            webview.connect_create(move |webview, action| {
                let url = action.request().and_then(|request| request.uri()).map(|uri| uri.as_str().to_string())?;
                match new_window_req_handler(
                    url.clone(),
                    NewWindowFeatures {
                        size: None,
                        position: None,
                        opener: NewWindowOpener { webview: webview.clone() },
                    },
                ) {
                    NewWindowResponse::Allow => {
                        ...
                        let window = gtk::ApplicationWindow::builder().application(&app).title(&url).build();
                        ...
                        Self::new_gtk(
                            &box_,
                            WebViewAttributes { ..Default::default() },
        ...
        W: IsA<gtk::Container>,
    {
        ...
        if container_type == "GtkBox" {
            container.dynamic_cast_ref::<gtk::Box>().unwrap().pack_start(webview, true, true, 0);
        } else if container_type == "GtkFixed" {
            let scale_factor = webview.scale_factor() as f64;
            let (width, height) =
                attributes.bounds.map(|b| b.size.to_logical::<i32>(scale_factor)).map(Into::into).unwrap_or((1, 1));
            let (x, y) =
                attributes.bounds.map(|b| b.position.to_logical::<i32>(scale_factor)).map(Into::into).unwrap_or((0, 0));
    ...
    fn attach_ipc_handler(webview: WebView, attributes: &mut WebViewAttributes) {
        ...
        let manager = webview.user_content_manager().expect("WebView does not have UserContentManager");
        ...
        manager.connect_script_message_received(None, move |_m, msg| {
            ...
            if let Some(js) = msg.js_value() {
                if let Some(ipc_handler) = &ipc_handler {
                    ipc_handler(
                        Request::builder().uri(webview.uri().unwrap().to_string()).body(js.to_string()).unwrap(),
                    );
                }
            }
        });
        ...
        if let Some(pending_scripts) = &mut *self.pending_scripts.lock().unwrap() {
            ...
            self.webview.evaluate_javascript(js, None::<&str>, None::<&str>, cancellable, |result| {
                ...
                if let Some(callback) = callback {
                    let result = result.map(|r| r.to_json(0)).unwrap_or_default().unwrap_or_default().to_string();
    ...
    fn init(&self, js: &str, for_main_only: bool) -> Result<()> {
        if let Some(manager) = self.webview.user_content_manager() {
            let script = UserScript::new(
                js,
                if for_main_only { UserContentInjectedFrames::TopFrame } else { UserContentInjectedFrames::AllFrames },
    ...
    fn cookie_into_soup_cookie(cookie: &cookie::Cookie<'_>) -> soup::Cookie {
        let mut soup_cookie = soup::Cookie::new(
            ...
            cookie.max_age().map(|d| d.whole_seconds() as i32).unwrap_or(-1),
        );
    ...
    pub fn cookies_for_url(&self, url: &str) -> Result<Vec<cookie::Cookie<'static>>> {
        ...
        if let Some(cookies_manager) = self.webview.website_data_manager().and_then(|manager| manager.cookie_manager())
        {
            cookies_manager.cookies(url, None::<&Cancellable>, move |cookies| {
                let cookies = cookies.map(|cookies| cookies.into_iter().map(Self::cookie_from_soup_cookie).collect());
    ...
    {
        if let Some(parent) = self.webview.parent().and_then(|p| p.dynamic_cast::<gtk::Container>().ok()) {
            parent.remove(&self.webview);

            let container_type = container.type_().name();
            if container_type == "GtkBox" {
                container.dynamic_cast_ref::<gtk::Box>().unwrap().pack_start(&self.webview, true, true, 0);
            } else if container_type == "GtkFixed" {
...
pub fn platform_webview_version() -> Result<String> {
    let (major, minor, patch) =
        unsafe { (webkit_get_major_version(), webkit_get_minor_version(), webkit_get_micro_version()) };
...
mod ffi {
    ...
    pub trait CookieManageExt: IsA<CookieManager> + 'static {
        fn all_cookies<P: FnOnce(std::result::Result<Vec<soup::Cookie>, glib::Error>) + 'static>(
            ...
            let has_acquired_main_context = (!is_main_context_owner).then(|| main_context.acquire().ok()).flatten();
            assert!(
            ...
            >(
                ...
                let ret = webkit_cookie_manager_get_all_cookies_finish(_source_object as *mut _, res, &mut error);
