...
use self::webview2::*;
#[cfg(target_os = "windows")]
use webview2_com::Microsoft::Web::WebView2::Win32::{ICoreWebView2, ICoreWebView2Controller, ICoreWebView2Environment};
...
#[cfg(target_os = "ios")]
pub type InputAccessoryViewBuilder = dyn Fn(&objc2_ui_kit::UIView) -> Option<Retained<objc2_ui_kit::UIView>>;
...
impl Default for Rect {
    fn default() -> Self {
        Self { position: dpi::LogicalPosition::new(0, 0).into(), size: dpi::LogicalSize::new(0, 0).into() }
    }
}
...
struct WebViewAttributes<'a> {
    ...
    pub custom_protocols: HashMap<String, Box<dyn Fn(WebViewId, Request<Vec<u8>>, RequestAsyncResponder)>>,
...
impl<'a> WebViewBuilder<'a> {
    ...
    pub fn new_with_web_context(web_context: &'a mut WebContext) -> Self {
        let attrs = WebViewAttributes { context: Some(web_context), ..Default::default() };
    ...
    pub fn with_initialization_script_for_main_only<S: Into<String>>(
        ...
        let script = js.into();
        if !script.is_empty() {
            self.attrs.initialization_scripts.push(InitializationScript { script, for_main_frame_only });
        }
        self
    }
    ...
    /// Set a handler closure to process the change of the webview's document title.
    pub fn with_document_title_changed_handler(mut self, callback: impl Fn(String) + 'static) -> Self {
    ...
    pub fn with_on_page_load_handler(mut self, handler: impl Fn(PageLoadEvent, String) + 'static) -> Self {
    ...
    pub fn build_as_child<W: HasWindowHandle>(self, window: &'a W) -> Result<WebView> {
        ...
        InnerWebView::new_as_child(window, self.attrs, self.platform_specific).map(|webview| WebView { webview })
    }
}
...
pub trait WebViewBuilderExtMacos {
    ...
    fn with_webview_configuration(self, configuration: Retained<objc2_web_kit::WKWebViewConfiguration>) -> Self;
}

#[cfg(target_os = "macos")]
impl WebViewBuilderExtMacos for WebViewBuilder<'_> {
    fn with_webview_configuration(mut self, configuration: Retained<objc2_web_kit::WKWebViewConfiguration>) -> Self {
        self.platform_specific.webview_configuration.replace(configuration);
        self
    }
}
...
impl WebViewBuilderExtIos for WebViewBuilder<'_> {
    ...
    >(
        ...
        self.platform_specific.input_accessory_view_builder.replace(Box::new(builder));
        self
    }
    fn with_limit_navigations_to_app_bound_domains(mut self, limit_navigations: bool) -> Self {
        self.platform_specific.limit_navigations_to_app_bound_domains = limit_navigations;
        self
    }
}
...
pub(crate) struct PlatformSpecificWebViewAttributes {
    on_webview_created: Option<
        std::sync::Arc<dyn Fn(prelude::Context) -> std::result::Result<(), jni::errors::Error> + Send + Sync + 'static>,
    >,
...
pub trait WebViewBuilderExtAndroid {
    fn on_webview_created<
        F: Fn(prelude::Context<'_, '_>) -> std::result::Result<(), jni::errors::Error> + Send + Sync + 'static,
    >(
        self,
        f: F,
    ) -> Self;
...
impl<'a> WebViewBuilderExtUnix<'a> for WebViewBuilder<'a> {
        ...
        W: gtk::prelude::IsA<gtk::Container>,
    {
        ...
        InnerWebView::new_gtk(widget, self.attrs, self.platform_specific).map(|webview| WebView { webview })
    }
...
impl WebViewExtAndroid for WebView {
    fn handle(&self) -> JniHandle {
        JniHandle { activity_id: self.webview.activity_id }
    }
}
