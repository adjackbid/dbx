...
static CHANNEL: Lazy<(Sender<(ActivityId, WebViewMessage)>, Receiver<(ActivityId, WebViewMessage)>)> =
    Lazy::new(|| bounded(8));
...
impl ActivityProxy {
    pub fn new(vm: JavaVM, activity: GlobalRef, window_manager: GlobalRef, webchrome_client: GlobalRef) -> Self {
        Self { activity, window_manager, webview: None, webchrome_client, java_vm: vm.get_java_vm_pointer() as *mut _ }
    }
}
...
pub fn activity_id_for_window_manager(window_manager: JObject) -> Option<ActivityId> {
    for (activity_id, proxy) in ACTIVITY_PROXY.lock().unwrap().iter() {
        ...
        let equals = env
            .call_method(proxy.window_manager.as_obj(), "equals", "(Ljava/lang/Object;)Z", &[(&window_manager).into()])
            .and_then(|v| v.z())
            .unwrap_or_default();
        if equals {
...
pub fn get_webview(activity_id: ActivityId) -> Option<GlobalRef> {
    ACTIVITY_PROXY.lock().unwrap().get(&activity_id).unwrap().webview.as_ref().cloned()
}
...
impl<'a> MainPipe<'a> {
    pub(crate) fn send(activity_id: ActivityId, message: WebViewMessage) {
        ...
        if CHANNEL.0.send((activity_id, message)).is_ok() {
            unsafe { libc::write(MAIN_PIPE[1].as_raw_fd(), &true as *const _ as *const _, size) };
        }
    }
            ...
            match message {
                WebViewMessage::CreateWebView(attrs) => {
                    ...
                    let webview = self.env.new_object(
                        ...
                        &[(&activity).into(), (&initialization_scripts_array).into(), (&id).into()],
                    )?;
                    // get settings
                    let web_settings =
                        self.env.call_method(&webview, "getSettings", "()Landroid/webkit/WebSettings;", &[])?.l()?;
                    // set media autoplay
                    ...
                    if javascript_disabled {
                        self.env.call_method(&web_settings, "setJavaScriptEnabled", "(Z)V", &[false.into()])?;
                    }
                    ...
                    let rust_webview_client_class = find_class(&mut self.env, &activity, client_class_name.clone())?;
                    ...
                    let ipc_class = find_class(&mut self.env, &activity, format!("{}/Ipc", PACKAGE.get().unwrap()))?;
                    ...
                    ACTIVITY_PROXY.lock().unwrap().get_mut(&activity_id).unwrap().webview.replace(webview);
                }
                WebViewMessage::Eval(script, callback) => {
                    if let Some(webview) = get_webview(activity_id) {
                        ...
                        #[cfg(feature = "tracing")]
                        let span =
                            std::sync::Mutex::new(Some(SendEnteredSpan(tracing::debug_span!("wry::eval").entered())));

                        EVAL_CALLBACKS.get_or_init(Default::default).lock().unwrap().insert(
                            id,
                            Box::new(move |result| {
                                #[cfg(feature = "tracing")]
                    ...
                    if let Some(activity) = activity_proxy(activity_id).map(|p| p.activity.clone()) {
                        match self
                            ...
                            .and_then(|s| {
                                ...
                                self.env.get_string(&s).map(|v| v.to_string_lossy().to_string())
                            }) {
                            Ok(version) => {
                ...
                WebViewMessage::ClearAllBrowsingData => {
                    if let Some(webview) = get_webview(activity_id) {
                        self.env.call_method(webview, "clearAllBrowsingData", "()V", &[])?;
                    }
                }
                ...
                WebViewMessage::GetCookies(tx, url) => {
                    if let Some(webview) = get_webview(activity_id) {
                        ...
                        tx.send(cookies.split("; ").flat_map(|c| cookie::Cookie::parse(c.to_string())).collect())
                            .unwrap();
                    }
                }
                WebViewMessage::OnDestroy { activity_id, webview_id, is_changing_configurations } => {
...
fn load_html<'a>(env: &mut JNIEnv<'a>, webview: &JObject<'a>, html: &JString<'a>) -> JniResult<()> {
    env.call_method(webview, "loadHTMLMainThread", "(Ljava/lang/String;)V", &[html.into()])?;
    Ok(())
}
...
fn set_background_color<'a>(env: &mut JNIEnv<'a>, webview: &JObject<'a>, (r, g, b, a): RGBA) -> JniResult<()> {
...
pub(crate) enum WebViewMessage {
    ...
    OnDestroy { activity_id: ActivityId, webview_id: WebviewId, is_changing_configurations: bool },
}
