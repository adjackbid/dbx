...
macro_rules! android_binding {
    ...
    ($domain:ident, $package:ident, $wry:path) => {{
        ...
        android_fn!($domain, $package, Rust, onWebviewDestroy, [JObject, JString]);

        android_fn!($domain, $package, Rust, handleRequest, [JString, JObject, jboolean], jobject);
        android_fn!($domain, $package, Rust, withAssetLoader, [JString], jboolean);
        android_fn!($domain, $package, Rust, assetLoaderDomain, [JString], jstring);
        android_fn!($domain, $package, Rust, shouldOverride, [JString, JString], jboolean);
        ...
        android_fn!($domain, $package, Rust, handleReceivedTitle, [JString, JString],);
    }};
}
...
fn handle_request(
    ...
    if let Some(handler) = REQUEST_HANDLER.lock().unwrap().get(webview_id) {
        ...
        let method = env.call_method(&request, "getMethod", "()Ljava/lang/String;", &[])?.l().map(JString::from)?;
        request_builder = request_builder.method(env.get_string(&method)?.to_string_lossy().to_string().as_str());

        let request_headers = env.call_method(request, "getRequestHeaders", "()Ljava/util/Map;", &[])?.l()?;
        ...
        while let Some((header, value)) = iter.next(env)? {
            ...
            if let (Ok(header), Ok(value)) =
                (HeaderName::from_bytes(header.to_bytes()), HeaderValue::from_bytes(value.to_bytes()))
            {
        ...
        let response = {
            ...
            (handler.handler)(webview_id, final_request, is_document_start_script_enabled != 0)
        };
        if let Some(response) = response {
            ...
            let (mime_type, encoding) = if let Some(content_type) = response.headers().get(CONTENT_TYPE) {
                ...
                (
                    ...
                    if let Some(encoding) = encoding { env.new_string(encoding)? } else { JString::default() },
                )
            } else {
...
pub unsafe fn onWebviewDestroy(mut env: JNIEnv, _: JClass, activity: JObject, webview_id: JString) {
    let activity_id = env.call_method(&activity, "getId", "()I", &[]).unwrap().i().unwrap();

    let webview_id = env.get_string(&webview_id).unwrap().to_string_lossy().to_string();

    let is_changing_configurations =
        env.call_method(&activity, "isChangingConfigurations", "()Z", &[]).unwrap().z().unwrap();

    super::MainPipe::send(
        activity_id,
        super::WebViewMessage::OnDestroy { activity_id, webview_id, is_changing_configurations },
    );
}
...
pub unsafe fn handleRequest(
    ...
    match handle_request(&mut env, webview_id, request, is_document_start_script_enabled) {
...
#[allow(non_snake_case)]
pub unsafe fn shouldOverride(mut env: JNIEnv, _: JClass, webview_id: JString, url: JString) -> jboolean {
...
pub unsafe fn ipc(mut env: JNIEnv, _: JClass, webview_id: JString, url: JString, body: JString) {
    match (env.get_string(&url), env.get_string(&body), env.get_string(&webview_id)) {
...
pub unsafe fn withAssetLoader(mut env: JNIEnv, _: JClass, webview_id: JString) -> jboolean {
    ...
    let webview_id = webview_id.to_str().ok().unwrap_or_default();
    (*WITH_ASSET_LOADER.lock().unwrap().get(webview_id).unwrap_or(&false)).into()
}
