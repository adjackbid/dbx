...
use crate::{
    custom_protocol_workaround, inject_initialization_scripts::inject_scripts_into_html, Error, RequestAsyncResponder,
    Result,
};
...
mod main_pipe;
use main_pipe::{
    activity_id_for_window_manager, first_activity_id, register_activity_proxy, ActivityId, CreateWebViewAttributes,
    MainPipe, WebViewMessage,
};
...
pub unsafe fn android_setup(package: &str, mut env: JNIEnv, _looper: &ThreadLooper, activity: GlobalRef) {
    PACKAGE.get_or_init(move || package.to_string());

    let vm = env.get_java_vm().unwrap();

    let activity_id = env.call_method(activity.as_obj(), "getId", "()I", &[]).unwrap().i().unwrap();

    let window_manager =
        env.call_method(&activity, "getWindowManager", "()Landroid/view/WindowManager;", &[]).unwrap().l().unwrap();
    ...
    let rust_webchrome_client_class =
        find_class(&mut env, activity.as_obj(), format!("{}/RustWebChromeClient", PACKAGE.get().unwrap())).unwrap();
    ...
    if let Some(webview_attributes) = WEBVIEW_ATTRIBUTES.lock().unwrap().get(&activity_id) {
        MainPipe::send(activity_id, WebViewMessage::CreateWebView(webview_attributes.clone()));
    }
}
...
impl InnerWebView {
    ...
    pub fn new(
        ...
        let window_manager = match window.window_handle()?.as_raw() {
            raw_window_handle::RawWindowHandle::AndroidNdk(window_manager) => window_manager.a_native_window,
            _ => return Err(Error::UnsupportedWindowHandle),
        };
        let window_manager = unsafe { JObject::from_raw(window_manager.as_ptr().cast()) };
        let activity_id = activity_id_for_window_manager(window_manager).expect("no available activity");
        ...
        let id = attributes.id.map(|id| id.to_string()).unwrap_or_else(|| COUNTER.next().to_string());

        WITH_ASSET_LOADER.lock().unwrap().insert(id.clone(), with_asset_loader);
        if let Some(domain) = asset_loader_domain {
            ASSET_LOADER_DOMAIN.lock().unwrap().insert(id.clone(), domain);
        }
      ...
      .insert(
        ...
        if let Some(i) = ipc_handler {
            IPC.lock().unwrap().insert(id.clone(), UnsafeIpc::new(Box::new(i)));
        }

        if let Some(i) = attributes.document_title_changed_handler {
            TITLE_CHANGE_HANDLER.lock().unwrap().insert(id.clone(), UnsafeTitleHandler::new(i));
        }

        if let Some(i) = attributes.navigation_handler {
            URL_LOADING_OVERRIDE.lock().unwrap().insert(id.clone(), UnsafeUrlLoadingOverride::new(i));
        }

        if let Some(h) = attributes.on_page_load_handler {
            ON_LOAD_HANDLER.lock().unwrap().insert(id.clone(), UnsafeOnPageLoadHandler::new(h));
        }
        ...
        WEBVIEW_ATTRIBUTES.lock().unwrap().insert(activity_id, attributes.clone());
    ...
    pub fn eval(&self, js: &str, callback: Option<impl Fn(String) + Send + 'static>) -> Result<()> {
        MainPipe::send(
            self.activity_id,
            WebViewMessage::Eval(js.into(), callback.map(|c| Box::new(c) as Box<dyn Fn(String) + Send + 'static>)),
        );
        Ok(())
    }
    ...
    pub fn set_background_color(&self, background_color: RGBA) -> Result<()> {
        MainPipe::send(self.activity_id, WebViewMessage::SetBackgroundColor(background_color));
        Ok(())
    }

    pub fn load_url(&self, url: &str) -> Result<()> {
        MainPipe::send(self.activity_id, WebViewMessage::LoadUrl(url.to_string(), None));
        Ok(())
    }

    pub fn load_url_with_headers(&self, url: &str, headers: http::HeaderMap) -> Result<()> {
        MainPipe::send(self.activity_id, WebViewMessage::LoadUrl(url.to_string(), Some(headers)));
        Ok(())
    }
    ...
    pub fn cookies_for_url(&self, url: &str) -> Result<Vec<cookie::Cookie<'static>>> {
        let (tx, rx) = bounded(1);
        MainPipe::send(self.activity_id, WebViewMessage::GetCookies(tx, url.to_string()));
        rx.recv_timeout(MAIN_PIPE_TIMEOUT).map_err(Into::into)
    }
...
pub fn find_class<'a>(env: &mut JNIEnv<'a>, activity: &JObject<'_>, name: String) -> JniResult<JClass<'a>> {
...
where
    F: FnOnce(&mut JNIEnv, &JObject, &JObject) + Send + 'static,
{
    MainPipe::send(first_activity_id().expect("no available activity"), WebViewMessage::Jni(Box::new(func)));
}
