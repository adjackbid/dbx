...
fn main() {
    ...
    if target_os == "android" {
        ...
        if let Ok(kotlin_out_dir) = std::env::var("WRY_ANDROID_KOTLIN_FILES_OUT_DIR") {
            ...
            let kotlin_out_dir = PathBuf::from(&kotlin_out_dir).canonicalize().unwrap_or_else(move |_| {
                panic!("Failed to canonicalize `WRY_ANDROID_KOTLIN_FILES_OUT_DIR` path {kotlin_out_dir}")
            });

            let kotlin_files_path = PathBuf::from(env_var("CARGO_MANIFEST_DIR")).join("src/android/kotlin");
            ...
            for file in kotlin_files {
                ...
                let class_extension_env = format!(
                    ...
                    file.path().file_stem().unwrap().to_string_lossy().to_uppercase()
                );
                let class_init_env =
                    format!("WRY_{}_CLASS_INIT", file.path().file_stem().unwrap().to_string_lossy().to_uppercase());
                ...
                let auto_generated_comment =
                    match file.path().extension().unwrap_or_default().to_str().unwrap_or_default() {
