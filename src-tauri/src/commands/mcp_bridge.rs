...
mod tests {
    ...
    use dbx_core::storage::{McpConnectionPolicy, McpDatabasePolicy, McpDatabaseScope, McpUserPolicy, Storage};
    use std::sync::Arc;
...
async fn load_mcp_policy_context(
    state: &Arc<AppState>,
) -> Result<(McpUserPolicy, HashMap<String, McpConnectionGroupPath>), String> {
...
fn ensure_database_in_mcp_scope(policy: &McpUserPolicy, connection_id: &str, database: &str) -> Result<(), String> {
