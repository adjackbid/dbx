...
async fn run_object_cache_regression(postgres: bool) {
    ...
    storage
        .save_mcp_user_policy("", &McpUserPolicy { read_only: false, allow_dangerous_sql: true, ..Default::default() })
        .await
        .unwrap();
    ...
    let router = Router::new()
        ...
        .layer(axum::middleware::from_fn(|mut req: axum::extract::Request, next: axum::middleware::Next| async move {
            // Production injects the signed-in session in the auth middleware; this test
            // router omits the middleware, so inject the same extension here.
            req.extensions_mut().insert(crate::state::UserSession {
                user_id: String::new(),
