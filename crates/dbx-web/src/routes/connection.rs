...
pub async fn mcp_duplicate_connection(
    ...
    let saved = state
        .app
        .storage
        .duplicate_connection_for_mcp(&body.source_id, &body.copy_id, &body.copy_name, &session.user_id)
        .await
        .map_err(AppError::from)?;
    state.app.session_credentials.clear_connection(&saved.id);
    state.app.configs.write().await.insert(saved.id.clone(), saved.clone());
...
mod tests {
    ...
    async fn mcp_connection_routes_preserve_unrelated_concurrent_changes() {
        ...
        assert!(persisted.iter().any(|config| config.id == added.id));
        let layout = state.app.storage.load_sidebar_layout_for_user("").await.unwrap().unwrap();
    ...
    async fn connect_db_sanitizes_global_config_and_scopes_password_by_session() {
        ...
        std::fs::File::create(&db_path).unwrap();
        {
            ...
            sessions.insert("token-a".to_string(), test_session());
            sessions.insert("token-b".to_string(), test_session());
        }
    ...
    async fn load_connections_drops_cached_pool_for_updated_config() {
        ...
        let result =
            load_connections(State(state.clone()), axum::extract::Extension(test_session()), HeaderMap::new()).await;
        assert!(result.is_ok());
    ...
    async fn sync_connection_configs_clears_all_owner_credentials_on_delete_and_id_reuse() {
        ...
        let reused = mcp_add_connection(
            State(state.clone()),
            axum::extract::Extension(test_session()),
            Json(McpAddConnectionRequest { config: recreated.clone() }),
        )
        .await
        .unwrap_or_else(|error| panic!("{}", error.message));
