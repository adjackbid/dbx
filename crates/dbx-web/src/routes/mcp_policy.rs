...
use dbx_core::storage::{McpDatabaseScope, McpUserPolicy};
...
async fn load_policy_context_for_connection(
    state: &Arc<WebState>,
    connection_id: &str,
) -> Result<(McpUserPolicy, HashMap<String, McpConnectionGroupPath>), AppError> {
    // The policy and the group paths both come from the connection's owning
    // account: accounts never see each other's MCP scope or sidebar groups.
    let policy = load_policy_for_connection(state, connection_id).await?;
    let owner = state.app.storage.connection_user_id(connection_id).await.map_err(AppError::from)?;
    let layout = match owner {
        Some(owner) => state.app.storage.load_sidebar_layout_for_user(&owner).await,
        None => state.app.storage.load_sidebar_layout().await,
    };
    let group_paths = match layout {
...
fn ensure_database_in_scope(policy: &McpUserPolicy, connection_id: &str, database: &str) -> Result<(), AppError> {
...
#[cfg(test)]
fn effective_database_execution_policy(policy: &McpUserPolicy, connection_id: &str, database: &str) -> (bool, bool) {
...
pub async fn ensure_scope(state: &Arc<WebState>, headers: &HeaderMap, connection_id: &str) -> Result<(), AppError> {
    ...
    let (policy, group_paths) = load_policy_context_for_connection(state, connection_id).await?;
...
mod tests {
    ...
    use dbx_core::storage::{McpConnectionPolicy, McpDatabasePolicy, McpDatabaseScope, McpUserPolicy};

    fn database_policy() -> McpUserPolicy {
        McpUserPolicy {
            read_only: true,
