...
use dbx_core::{models::connection::ConnectionConfig, storage::McpUserPolicy, storage::Storage};
...
fn full_access_policy() -> McpUserPolicy {
    McpUserPolicy {
        read_only: false,
...
fn short_timeout_policy() -> McpUserPolicy {
    McpUserPolicy {
        read_only: false,
        allow_dangerous_sql: true,
        allowed_connection_ids: None,
...
macro_rules! boot_live_server {
    ($connections:expr, $policy:expr) => {{
        ...
        storage.save_mcp_user_policy("", &$policy).await.expect("save policy");
        drop(storage);
