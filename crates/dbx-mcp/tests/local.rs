...
async fn duplicate_connection_preserves_secrets_ssh_and_sidebar_group() {
    ...
    storage
        .save_mcp_user_policy(
            "",
            &dbx_core::storage::McpUserPolicy {
                read_only: false,
    ...
    let backend = Arc::new(LocalBackend::open(&db_path).await.expect("open local backend"));
    let policy = backend.load_mcp_policy().await.expect("load configured policy");
    ...
    let connections = reopened.load_all_connections().await.expect("reload connections");
