...
use dbx_core::{
    ...
    storage::{McpDatabaseScope, McpUserPolicy},
};
...
fn policy_allows_tool(policy: &McpUserPolicy, tool_name: &str) -> bool {
...
mod tests {
    ...
    struct FakeBackend {
        ...
        policy_override: std::sync::Mutex<Option<McpUserPolicy>>,
    }
    ...
    async fn peek_messages_enforces_tool_connection_scope_and_kafka_only() {
        for (policy, conn, expected) in [
            (
                McpUserPolicy { allowed_tool_names: Some(vec![]), ..Default::default() },
                kafka_connection(),
                "TOOL_OUT_OF_SCOPE",
            ),
            (
                McpUserPolicy { allowed_connection_ids: Some(vec![]), ..Default::default() },
                kafka_connection(),
                "CONNECTION_OUT_OF_SCOPE",
            ),
            (McpUserPolicy::default(), connection("kafka", "redis", "redis", "0"), "MQ_UNSUPPORTED"),
        ] {
    ...
    fn confirmed_sql_binding_cannot_elevate_central_policy() {
        ...
        let read_only_policy = McpUserPolicy {
            read_only: true,
            allow_dangerous_sql: false,
            allowed_connection_ids: None,
            ..Default::default()
    ...
    async fn opted_in_session_exposes_transaction_tools_and_structured_state() {
        let policy = McpUserPolicy { read_only: false, allow_dangerous_sql: true, ..Default::default() };
        let backend = Arc::new(FakeBackend {
    ...
    async fn queued_transaction_query_rechecks_revoked_policy_before_io() {
        let policy = McpUserPolicy { allow_dangerous_sql: true, ..Default::default() };
        let backend = Arc::new(FakeBackend {
            connections: vec![connection("mysql", "mysql", "mysql", "app")],
            policy: policy.clone(),
            ..Default::default()
    ...
    async fn execute_batch_rejects_mysql_family_ddl_with_transaction() {
        ...
        let postgres_backend = Arc::new(FakeBackend {
            ...
            policy: McpUserPolicy { read_only: false, allow_dangerous_sql: true, ..Default::default() },
            ..Default::default()
        });
        let postgres_server = DbxMcpServer::with_runtime_options(postgres_backend.clone(), McpScope::default(), false);
