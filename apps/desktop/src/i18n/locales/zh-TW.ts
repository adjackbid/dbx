...
export default withEnglishFallback({
  ...
  settings: {
    ...
    mcpConfig: "MCP 設定",
    mcpConfigOptionsHint: "連線範圍與執行權限依登入帳號各自設定，並對以該帳號連線的 MCP 用戶端立即生效；產生的設定只包含啟動 MCP Server 與連線目前 DBX 所需的執行參數，不包含權限或連線範圍變數。",
    mcpScopeConnection: "允許 MCP 存取的連線",
    mcpScopeConnectionDescription: "DBX 只向 MCP 公開此帳號選取的連線，變更後對以此帳號連線的 MCP 用戶端生效。",
    mcpScopeMode: "連線範圍模式",
    ...
    mcpExecutionMode: "MCP 執行權限預設等級",
    mcpExecutionModeDescription: "DBX 對此帳號的 MCP 用戶端強制執行的預設等級；連線與單庫設定可依範圍覆蓋。",
