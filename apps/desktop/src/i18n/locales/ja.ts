...
export default withEnglishFallback({
  ...
  settings: {
    ...
    jdbcTab: "JDBCドライバー",
    securityTab: "セキュリティ",
    adminOnlySetting: "この設定は管理者のみ変更できます。",
    aboutTab: "バージョン情報",
    ...
    mcpConfig: "MCP設定",
    mcpConfigOptionsHint: "接続範囲と実行権限はログイン中のアカウントごとに設定され、そのアカウントで接続するMCPクライアントに即時適用されます。生成される設定にはMCP Serverの起動と現在のDBXへの接続に必要な実行パラメーターのみが含まれ、権限や接続範囲の変数は含まれません。",
    mcpScopeConnection: "MCPに許可する接続",
    mcpScopeConnectionDescription: "DBXはこのアカウントで選択した接続だけをMCPに公開します。変更はこのアカウントで接続するMCPクライアントに適用されます。",
    ...
    mcpExecutionModeDescription: "DBXがこのアカウントのMCPクライアントに適用する権限レベルを選択します。クライアント側の読み書き権限設定では、このレベルを変更できません。",
