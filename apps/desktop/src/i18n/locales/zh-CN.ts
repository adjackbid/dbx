...
export default withEnglishFallback({
  ...
  settings: {
    ...
    mcpConfig: "MCP 配置",
    mcpConfigOptionsHint: "连接范围和执行权限按登录账号分别设置，并对以该账号连接的 MCP 客户端立即生效；生成配置只包含启动 MCP Server 和连接当前 DBX 所需的运行参数，不包含权限或连接范围变量。",
    mcpScopeConnection: "允许 MCP 访问的连接",
    mcpScopeConnectionDescription: "DBX 只向 MCP 暴露此账号选中的连接，修改后对以该账号连接的 MCP 客户端生效。",
    mcpScopeMode: "连接范围模式",
    ...
    mcpExecutionMode: "MCP 执行权限默认等级",
    mcpExecutionModeDescription: "DBX 对此账号的 MCP 客户端强制执行的默认等级；连接与单库设置可依范围覆盖。",
