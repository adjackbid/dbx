...
const mcpAllowedGroupIds = computed(() => settingsStore.mcpUserPolicy.allowedGroupIds);
const mcpQueryTimeoutInput = ref<string>(settingsStore.mcpUserPolicy.queryTimeoutSecs === null ? "" : String(settingsStore.mcpUserPolicy.queryTimeoutSecs));

watch(
  () => settingsStore.mcpUserPolicy.queryTimeoutSecs,
  (value) => {
...
function onMcpQueryTimeoutInput(event: Event) {
  ...
  if (raw === "") {
    ...
    if (!Number.isFinite(parsed) || parsed < 0 || !Number.isInteger(parsed) || parsed > Number("18446744073709551615")) {
      ...
      const reverted = settingsStore.mcpUserPolicy.queryTimeoutSecs === null ? "" : String(settingsStore.mcpUserPolicy.queryTimeoutSecs);
...
const mcpAllowedToolNames = computed(() => settingsStore.mcpUserPolicy.allowedToolNames);
...
function mcpConnectionExecutionMode(connectionId: string): McpConnectionExecutionMode | "inherit" {
  const rule = settingsStore.mcpUserPolicy.connectionPolicies.find((item) => item.connectionId === connectionId);
...
function mcpGroupExecutionMode(groupId: string): McpConnectionExecutionMode | "inherit" {
  const rule = settingsStore.mcpUserPolicy.groupPolicies.find((item) => item.groupId === groupId);
...
const mcpPermissionPreviewRows = computed(() =>
  mcpConnectionPolicyConnections.value.flatMap((connection) => {
    const rule = settingsStore.mcpUserPolicy.connectionPolicies.find((item) => item.connectionId === connection.id);
...
function onMcpGroupExecutionModeChange(groupId: string, mode: McpConnectionExecutionMode | "inherit") {
  ...
  const groupPolicies = settingsStore.mcpUserPolicy.groupPolicies.filter((item) => item.groupId !== groupId);
  if (mode !== "inherit") {
    groupPolicies.push({
      groupId,
...
function onMcpConnectionExecutionModeChange(connectionId: string, mode: McpConnectionExecutionMode | "inherit") {
  ...
  const existing = settingsStore.mcpUserPolicy.connectionPolicies.find((item) => item.connectionId === connectionId);
  const rules = settingsStore.mcpUserPolicy.connectionPolicies.filter((item) => item.connectionId !== connectionId);
...
function onMcpConnectionPoliciesChange(connectionPolicies: typeof settingsStore.mcpUserPolicy.connectionPolicies) {
                    ...
                    <template #connections>
                      <div class="space-y-3">
                        ...
                        <McpResourceScopePicker
                          ...
                          :group-policies="settingsStore.mcpUserPolicy.groupPolicies"
                          :connection-policies="settingsStore.mcpUserPolicy.connectionPolicies"
